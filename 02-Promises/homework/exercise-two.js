...
function problemA () {
  ...
  Promise.all([
  promisifiedReadFile('poem-two/stanza-01.txt').then(stan1=>blue(stan1)),
  promisifiedReadFile('poem-two/stanza-02.txt').then(stan2=>blue(stan2))
...
function problemB () {
  ...
  // promise version
  // ???
const arrayPromesas=filenames.map(file=>promisifiedReadFile(file).then(blue))

Promise.all(arrayPromesas).then(()=>console.log('done'))
}
...
function problemC () {
  ...
  // promise version
  // ???
 let promesaActual=promisifiedReadFile(filenames[0])

 for(let i=1; i<=filenames.length; i++){
 promesaActual=promesaActual.then(stanza=>{
 blue(stanza)
  ...
  function promisifiedWriteFile (filename, str) {
    ...
    return new Promise((resolve, reject) => {
      fs.writeFile(filename, str,err=> {
        if (err) reject(err);
        else resolve('Funcionando');
      });
    })
    
  }

  promisifiedWriteFile('prueba.txt','pt-06')
  .then(console.log)
